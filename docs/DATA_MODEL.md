...
`openclaw_owner` owns schema objects. `openclaw_runtime` is created by `000_roles.sh`; its grants begin in `002_runtime_grants.sql` and are extended by the later migrations that add tables, sequences, or functions it must reach (`004`, `005`, `006`, `009`, `010`, `012`, `013`, `014`, `015`, `016`, `017`, `018`). Two of those also *narrow* the surface: `004` revokes `UPDATE` on `facts`, `fact_sources`, `document_facts`, `compiled_truth_facts` and `evaluation_criteria`, and `017` completes the same append-only privilege contract by revoking `UPDATE` on `contradiction_facts` and `trajectory_points`. The runtime therefore holds `SELECT`/`INSERT` but no `UPDATE` on all seven of those history tables, so the grant catalogue agrees with the triggers that already refused those writes. It receives no grant outside that reviewed migration series. One caveat on how that is achieved: migration 002's `ALTER DEFAULT PRIVILEGES … REVOKE ALL ON FUNCTIONS FROM PUBLIC` does not actually withhold `EXECUTE` from functions created later — PostgreSQL grants that to `PUBLIC` as a built-in default rather than a `pg_default_acl` entry, so there is nothing for the statement to revoke. Nothing is exposed by it today, but the reason is narrower than "no later function is reachable": every function a migration after `002` *defines itself* — as distinct from the members `CREATE EXTENSION` installs, covered below — that carries no explicit grant is a trigger function, so PostgreSQL's `PUBLIC` default buys a caller nothing without trigger context. `register_schema_migration` is not among them — it is created in `001`, so `002`'s blanket revoke reaches it, and it is `SECURITY INVOKER`, so the runtime's `SELECT`-only grant on `schema_migrations` refuses its INSERT regardless; and the twelve non-trigger `SECURITY DEFINER` functions all end up with an explicit `REVOKE`/`GRANT` pair, by two different routes. The eight created in `001` predate `002`, so their `PUBLIC` execute is withdrawn by `002`'s blanket `REVOKE ALL ON ALL FUNCTIONS IN SCHEMA public FROM PUBLIC` — which, unlike the `ALTER DEFAULT PRIVILEGES` line, applies at execution time to functions that already exist — and they are granted individually in the same migration. The four created after `002` (`promote_submitted_claim` in 012, `decide_proposal` in 015, `consume_approval_and_erase_lead` in 016/017, `retry_workflow_run` in 018) each carry their own `REVOKE ALL … FROM PUBLIC` and `GRANT EXECUTE … TO openclaw_runtime` pair in the migration that creates them, which is the only route that works after `002`. `tests/v3/test_runtime_grant_enumeration.py` reads the resulting schema and fails the offline gate if a `SECURITY DEFINER` function is ever missing **either** half. Write both in the migration that creates the function; do not rely on the default-privileges line. The one place the caveat has an observable consequence is migration 009, which creates the `pg_trgm` extension after 002: twenty-nine of its thirty-one member functions keep PostgreSQL's built-in `EXECUTE TO PUBLIC` (`show_trgm`, `set_limit`, `show_limit` and the GiST/GIN support functions included), so `openclaw_runtime` can call them even though 009 names only `similarity` and `similarity_op` — from which it revokes the `PUBLIC` default explicitly before re-granting them to the runtime. All thirty-one are `SECURITY INVOKER` and read no package data, so nothing is exposed — but migration 009's own comment claiming the runtime "receives no extension-management or inspection functions" describes an intent that neither statement can enforce for functions an extension creates; the migration is immutable after release, so this paragraph is the correction.

Migration files are immutable once deployed. The one exception on record is the open-source publication of 3.0.0, which re-stamped the licence and project-name header lines of all eighteen files while no deployment existed outside the development host; `docs/RUNBOOK.md` §1 records what that broke and why there is no upgrade path across it. The installer computes each file's lowercase SHA-256 outside Postgres, applies it in a transaction, and then calls:
