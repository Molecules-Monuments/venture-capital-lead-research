...
(**348 tests, 30/30** base checks), and each opt-in gate individually —
...
audit remediation added one hundred and thirty-eight in total; the per-suite figures in the table
...
| --- | ---: | --- |
| Offline verification (the nine declared suites listed under "Passing evidence" below, ruff, ty, shell syntax, fixed workflows, skill system, manifest currency, pristine) | **PASS — 348 tests, 30/30 checks** | `python3 -B scripts/verify_offline.py` |
...
| Recovery/release lifecycle | 41/41 | `python3 -B -m unittest discover -s tests/g7 -p 'test*.py' -v` |
...
The aggregate deterministic offline suites pass 348 tests with no failures or
