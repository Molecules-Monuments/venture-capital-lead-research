...
2026-08-18**, against this tree and against the derived
image rebuilt from it with `docker build --no-cache --pull` on **2026-08-18**,
...
(**346 tests, 30/30** base checks), and each opt-in gate individually —
`run_g4.py` (**97/97** across seven suites, migrations 001–018 applied
...
audit remediation added one hundred and thirty-six in total; the per-suite figures in the table
...
| --- | ---: | --- |
| Offline verification (the nine declared suites listed under "Passing evidence" below, ruff, ty, shell syntax, fixed workflows, skill system, manifest currency, pristine) | **PASS — 346 tests, 30/30 checks** | `python3 -B scripts/verify_offline.py` |
| Disposable Postgres hard gate | **PASS — 97/97** across seven suites, migrations 001–018 applied twice | `python3 -B scripts/run_g4.py` |
...
## Passing evidence (executed 2026-07-23; last re-executed 2026-08-18)
...
| Version 3 providers/context/orchestration/customization/skill system | 130/130 | `python3 -B -m unittest discover -s tests/v3 -p 'test*.py' -v` |
...
| Infrastructure contracts | 37/37 | `python3 -B -m unittest discover -s tests/infrastructure -p 'test*.py' -v` |
...
| Recovery/release lifecycle | 40/40 | `python3 -B -m unittest discover -s tests/g7 -p 'test*.py' -v` |
...
| Data/helper/Postgres hard gate | **97/97** | `python3 -B scripts/run_g4.py` |
| Real deployment gate | PASS | `python3 -B scripts/run_g8_deployment.py` (or `verify_offline.py --with-deployment`) |

The nine `unittest discover` rows above are the suite inventory
`scripts/verify_offline.py` declares in its `SUITES` tuple, and
`tests/v3/test_evidence_doc_consistency.py` fails if a declared suite loses its
row here. That declaration is what the gate runs: a test directory `SUITES`
does not name is not executed by `verify_offline.py`, so a new suite has to be
added to `SUITES` and to this table together.

The aggregate deterministic offline suites pass 346 tests with no failures or
...
re-run on 2026-08-18 against an image rebuilt from this tree with `docker build
--no-cache --pull` (8/8), and the retrieval-scale gate was re-run on 2026-08-06,
2026-08-07, 2026-08-08, 2026-08-09, 2026-08-10, 2026-08-11, 2026-08-16 and
2026-08-18 (160/160 cases
every time).
