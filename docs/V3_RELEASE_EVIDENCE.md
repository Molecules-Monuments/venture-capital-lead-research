...
(**358 tests, 30/30** base checks), and each opt-in gate individually —
...
audit remediation added one hundred and forty-eight in total; the per-suite figures in the table
...
| --- | ---: | --- |
| Offline verification (the nine declared suites listed under "Passing evidence" below, ruff, ty, shell syntax, fixed workflows, skill system, manifest currency, pristine) | **PASS — 358 tests, 30/30 checks** | `python3 -B scripts/verify_offline.py` |
...
| Version 3 providers/context/orchestration/customization/skill system | 135/135 | `python3 -B -m unittest discover -s tests/v3 -p 'test*.py' -v` |
...
The aggregate deterministic offline suites pass 358 tests with no failures or
