...
- `vc-quarantine` — copies of uploads the extract lane rejected *and could
  ...
  nothing (the read lane performs no mutation), and even in the extract lane an
  oversized or unreadable input yields `materialized: false` and no copy. The
  volume can therefore be empty after real rejections; `RUNBOOK.md` §9,
  "Malicious document", says how to tell;
