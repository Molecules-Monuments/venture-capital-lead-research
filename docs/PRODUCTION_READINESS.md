...
`scripts/set_evidence_execution_date.py`. They name the measurement of a **2026-08-20**
...
with `docker build --no-cache --pull` on 2026-08-25, after that day's edits to the image-baked `workspaces/` files, `Dockerfile.openclaw` itself, and the host-side recovery-lifecycle scripts. Both
