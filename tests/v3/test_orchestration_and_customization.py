...
# The band-edge customisation surface is deliberately NOT machine-enumerated.
#
# Rounds 6 and 7 each shipped a tree-scanning detector here that walked every
# file looking for band-edge arithmetic and required each hit to be named in
# CUSTOMIZATION.md or dispositioned. Both versions produced BLOCKERS of the same
# shape, and it is a shape worth naming: a detector over the whole tree cannot
# distinguish package content from OPERATOR content, and this package hands
# operators `inbox/`, `quarantine/` and a dozen MUST_CUSTOMIZE policy files and
# tells them to write whatever they like in them.
#
# Measured on the shipped tree: a one-line note in `inbox/` mentioning a band and
# a number failed the gate; so did customising `workspaces/vc-chief/USER.md`,
# `thesis.md` and `prequalification.md` -- all of which CUSTOMIZATION.md
# instructs operators to rewrite. Round 6's version additionally failed on the
# band change itself, in both documented branches, with no legal remedy.
#
# A gate that refuses a documented, mandatory operator action is worse than no
# gate: it is a gate operators learn to override. CUSTOMIZATION.md's band row
# still names every artifact that carries the shipped edges -- that documentation
# fix stands on its own and is the thing an operator actually follows -- but the
# enumeration is a review duty, not an offline check.
#
# If this is ever attempted again: the world must be the DECLARED package
# inventory (manifest.json) minus every operator-editable path, not the
# filesystem; and the customisation procedure must be walked end to end with the
# gate required green at the end of it.
