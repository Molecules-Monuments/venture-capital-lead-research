...
| `runtime-packages/**`, image/package locks, `deployment-lock.json` | Pin exact versions, then regenerate the lock with the two-pass procedure (`npm install --package-lock-only --omit=dev --allow-remote=all`, then `npm install --omit=dev --omit=peer --ignore-scripts --no-audit --no-fund`) — a single pass yields a lock `npm ci` rejects. Rebuild the image and repeat the supply-chain and G6 gates. |
