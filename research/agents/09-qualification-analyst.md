...
The counterweight is the power-law and false-negative problem. Korteweg and Sørensen find VC performance especially noisy and difficult to separate from luck ([paper](https://finance.darden.virginia.edu/wp-content/uploads/2018/01/paper-Sorensen.pdf)); **grade B/C**, fund-level and not a criterion experiment. Bessemer’s anti-portfolio likewise shows that reasonable filters can reject enormous outcomes, but is not a denominator-complete dataset. A model that predicts past investment choices can merely automate the fund’s biases. A published scorecard/ML case reports 78% accuracy in reproducing one firm’s decisions ([Systems](https://www.mdpi.com/2079-8954/9/3/55)); **grade C/D** because the target is historical choice, not net return, and the sample is one firm.
