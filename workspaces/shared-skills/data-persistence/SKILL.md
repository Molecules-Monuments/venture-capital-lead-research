...
| `capture_candidate` (`lead-signal-detector`, `inbound-intake-analyst`) | **not yet persisted, and not executable as handed over**: the `persistenceRequest` object those schemas define is `requested`/`operation`/`reason`/`steward_required` and nothing else (`additionalProperties: false`), so it carries no `lead_title`, `company_name`, `company_domain`, `origin_subtype`, or `idempotency_key`, and no resolver decision. Refuse it — but report `availability: fixed_workflow` with `mode: read_only`, **not** `operator_only`, and name the lane the chief must supply as a bounded assignment once `lead-memory-lookup` has cleared the duplicate: `inbound-text-intake` for a text-only founder or referrer submission, `outbound-scout` for a discovered candidate (one invocation per candidate) |
| `update_candidate` | classify `operator_only` and refuse, naming the operation. A new dated fact about a lead that already exists is a `record_evidence` request instead of this one |
| `record_document_claims` | classify `operator_only` and refuse, naming the operation |
