...
### Documentation corrected after the first tag write

Found by the acceptance review that cleared this release, deferred as
non-blocking, and closed on 2026-09-02 against a live deployment of the
shipped image. No code, configuration or image content changed, so `VERSION`
stays `3.0.1`; the `v3.0.1` tag moves to the commit carrying these
corrections, as this file's versioning note describes.
